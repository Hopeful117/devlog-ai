...
describe('EngineeringEventService', () => {
  ...
  it('loads the paged Project history and immutable detail', () => {
    ...
    expect(
      http.expectOne('http://core.test/api/v1/projects/project-id/engineering-events?page=1&size=5')
        .request.method,
    ).toBe('GET');
    expect(
      http.expectOne('http://core.test/api/v1/engineering-events/event%2Fid').request.method,
    ).toBe('GET');
  });
});
