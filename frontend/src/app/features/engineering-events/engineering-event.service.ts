...
import { EngineeringEvent, EngineeringEventExecution, EngineeringEventPage } from './engineering-event.models';

@Injectable({ providedIn: 'root' })
export class EngineeringEventService {
  private readonly http = inject(HttpClient);
  private readonly base = `${inject(APP_ENVIRONMENT).backendBaseUrl}/api/v1`;
  execute(projectId: string, sourceId: string, targetCommit: string): Observable<EngineeringEventExecution> {
  ...
  get(id: string): Observable<EngineeringEvent> {
    return this.http.get<EngineeringEvent>(`${this.base}/engineering-events/${encodeURIComponent(id)}`);
  }
}
