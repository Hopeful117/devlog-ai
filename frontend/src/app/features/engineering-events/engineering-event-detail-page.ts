  ...
  template: ` <article>
    @if (view$ | async; as view) {
      @if (view.state === 'loading') {
        <p role="status">Loading Engineering Event…</p>
      }
      @if (view.state === 'error') {
      ...
      @if (view.state === 'loaded') {
        ...
        <p>
          <strong>{{ view.event.category.replaceAll('_', ' ') }}</strong> ·
          {{ view.event.occurredAt | date: 'medium' }}
        </p>
        <p>{{ view.event.summary }}</p>
        <h2>Why this evolution is significant</h2>
        <p>{{ view.event.significance }}</p>
        <h2>Immutable revision boundary</h2>
        <code>{{ view.event.baseCommit }}</code
        ><span> → </span><code>{{ view.event.targetCommit }}</code>
        @if (view.event.mergeCommit) {
          <p>Merge commit compared through its first parent.</p>
        }
        <h2>Audit</h2>
        ...
        <ul>
          @for (reference of view.event.evidenceReferences; track reference) {
            <li>
              <code>{{ reference }}</code>
            </li>
          }
